...
def logistic_q2v(x, a, b, c):
    return c / (1 + a/1000 * np.exp(-b/100 * x))

def logistic_v2q(x, a, b, c, d):
    return (c * 100 / (1 + a/10000 * np.exp(-b/100 * x))) + d * 100

def test(x):
    return 99.37 / (1 + 0.0056 * np.exp(0.07586 * x))

class LogQ2VCurve:
    def __init__(self, x, y):
        (a_, b_, c_), _ = opt.curve_fit(logistic_q2v, x, y, maxfev=10000, p0=[0.005, 0.07, 99.0], bounds=(-100, 100))
        self.a = a_
        self.b = b_
        self.c = c_
    
    def __call__(self, *args, **kwargs):
        return logistic_q2v(args[0], self.a, self.b, self.c)
    
    def inverse(self, vmaf):
        inner = -(self.a / 1000) * vmaf / (vmaf - self.c)
        q = math.log(max(0.001, inner)) / (self.b / 100)
        return q


class LogV2QCurve:
    def __init__(self, x, y):
        # , p0=[2.6, -6.5, 323, -257], bounds=(-100, 100)
        # , p0 = [2.6, -6.5, 323, -257], bounds = (-100, 100)
        (a_, b_, c_, d_), _ = opt.curve_fit(logistic_v2q, x, y, maxfev=10000, p0=[2.6, -6.5, 3, -2], method='trf')
        self.a = a_
        self.b = b_
        self.c = c_
        self.d = d_
    ...
    def __call__(self, *args, **kwargs):
        return logistic_v2q(args[0], self.a, self.b, self.c, self.d)
...
class VideoDataChunk:
    def __init__(self, name, qs, vmafs):
        ...
        self.vmafs = vmafs
        self.probes = 0
        self.q_to_vmaf = LogQ2VCurve(self.qs, self.vmafs)
        self.vmaf_to_q = LogV2QCurve(self.vmafs, self.qs)
...
if __name__ == '__main__':
    ...
    for fname in vmaf_dir_files:
        ...
        for arg in ARG_LIST:
            ...
            cq_real = video_data_chunk.q_to_vmaf.inverse(arg.vmaf_target)
            cq_real = np.clip(cq_real, arg.min_q, arg.max_q)
    ...
    performed = len(ARG_LIST) * len(vmaf_dir_files)
    print(f"Performed {performed} target vmaf searches")
    print(f"Average Probes: {total_probes / performed}")
