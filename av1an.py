...
class Av1an:
    ...
    def encode(self, commands):
        ...
        try:
            ...
            # Queue execution
            for i in commands[:-1]:
                    tqdm_bar(i, self.encoder, counter, frame_probe_source, self.passes, self.log)
    ...
    def video_encoding(self):
        ...
        if self.resume and (self.temp / 'done.json').exists():
            ...
            framenums = split_routine(self.input, self.scenes, self.split_method, self.temp, self.min_scene_len, self.queue, self.threshold, self.ffmpeg)
